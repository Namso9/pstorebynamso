...
- **FAQ/review source-of-truth hardening is DEPLOYED (2026-08-09, `af443c4`,
  Cloudflare `c5683eff-101d-493a-9e65-2997e0686377`).** Canonical sources are
  ...
  and review-list mutations now trigger Pages builds, while the live `/data`
  Functions serve canonical GitHub JSON immediately with a five-second rolling
  upstream key and `no-store`. Visible React consumers revalidate every five
  seconds and on focus. The sale bot owns no FAQ copy/cache and links its FAQ
  action to this storefront. Combined source/mirror, proxy, Review, 36 Admin
  tests, dependency audit, lint, typecheck, production build, CSP, Zoom, and
  49-item/98-open browser FAQ checks pass locally and on production. Live FAQ
  bytes equal canonical GitHub, and review31 is `github-live`/no-store.
- **Canonical sequential review flow is DEPLOYED (2026-08-09).**
  ...
  Final production is `af443c4` / `c5683eff-101d-493a-9e65-2997e0686377`;
