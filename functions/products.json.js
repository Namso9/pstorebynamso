 ...
 * stock/price commit ဖြစ်တာနဲ့ စက္ကန့်အနည်းငယ်အတွင်း site မှာ live ဖြစ်တယ်၊
...
export async function onRequestGet({ request, env }) {
  try {
    const body = await fetchLiveJson(RAW_URL);
    return new Response(body, {
      headers: freshJsonHeaders('X-Products-Source', 'github-live'),
    });
  } catch (e) {
    // fall through to the static copy
  }
  const fallback = await env.ASSETS.fetch(request);
  return freshFallbackResponse(fallback, 'X-Products-Source');
}
